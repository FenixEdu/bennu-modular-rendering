...
public class Intersection {
	...
	private Intersection(String location, String position, int priority,
	 ...
	 *            a map of arguments.
	 */
	public static void generate(final String location, final String position,
			Map<String, Object> args, final Writer writer) {
		String pos;
		if (position == null) {
			pos = "";
		} else {
		...
		Map<String, Object> arguments = Collections
				.unmodifiableMap(possibleArgs);

		// First order by priority.
		Comparator<Intersection> priority = Comparator
				.comparingInt(x -> x.priority);

		// if two locations are equal, it should retain a stable order,
		// therefore ordering by location
		final Comparator<Intersection> comp = priority.thenComparing(
				x -> x.location + ":" + x.position,
		...
		Optional.ofNullable(INTERSECTIONS.get(getKey(location, pos)))
				.ifPresent(
						handlers -> handlers
								...
								.map((x) -> {
									try {
										return x.callback.apply(
												new IntersectionEvent(location,
														pos, x.priority),
												arguments);
									} catch (Throwable e) {
										LOGGER.error(
												"Error while processing partial",
												e);
									}
								...
								.forEach(
										x -> {
											try {
												...
												LOGGER.error(
														"Error while writing partial to writer",
														e);
											}
										}));
	}
	 ...
	 */
	public static RegistrationHandler at(String location, String position,
			Integer priority, IntersectionHandler callback) {
		if (position == null) {
			position = "";
		}
		...
		List<Intersection> callbacks = INTERSECTIONS.get(getKey(location,
				position));
	 ...
	 * {@link org.fenixedu.bennu.rendering.Intersection#at(String, String, Integer, IntersectionHandler)}
	 ...
	 *            the actualy callback
	 */
	public static RegistrationHandler at(String location,
			IntersectionHandler callback) {
	...
	public class RegistrationHandler {
		...
		public void unregister() {
			List<Intersection> intersections = INTERSECTIONS.get(getKey(
					Intersection.this.location, Intersection.this.position));
